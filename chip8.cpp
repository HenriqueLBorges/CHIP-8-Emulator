...
/*
	...
	al_register_event_source(fila_eventos, al_get_display_event_source(janela)); //Prepara a fila de evenos para pegar qualquer evento relacionado e janela
...
void chip8::emula_Ciclo()
{
	...
	case 0x8000:
		...
		case 0x0005: // 0x8XY5 Subtrai os registradores [Y] de [X] e guarda em [X]. Registrador [15] = 1 se tiver borrow.
			if (registrador[(codigo_op & 0x0F00) >> 4] > registrador[(codigo_op & 0x00F0) >> 8]) //Verifica o conteudo dos registradores [X] e [Y]
	...
	case 0xC000: // 0xCXNN: Seta o registrador [X] com o resultado da operacao entre NN e um numero aleatorio (0 e 255)
					...
					//Le a imagem da memoria
		{
			...
			registrador[15] = 0; //flag de colisao
