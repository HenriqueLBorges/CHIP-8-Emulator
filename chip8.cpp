...
void chip8::emula_Ciclo()
{
	...
	codigo_op = memoria[PC] << 8 | memoria[PC + 1]; //adiciona 8 zeros e usa a opera��o l�gica OR para juntar a instrucao que est� dividida em memoria[PC] e memoria [PC+1]
	...
	switch (codigo_op & 0xF000) { //Utiliza a opera��o l�gica AND para fazer uma m�scara de bits que ir� verificar apenas os 4 bits da esquerda para direita (os bits que contem a opera��o)
		...
		case 0x0000:
			switch (codigo_op & 0x000F)//m�scara de bits para verificar os 4 �ltimos e determinar a instrucao
			{
		...
		case 0x1000:// 0x1NNN: Pula para o endereco NNN
			PC = codigo_op & 0x0FFF; //M�scara de bits para pegar o endereco
		break;
		...
		case 0x3000:// 0x3XNN: Pula a instrucao se o Registrador [X] for igual a NN
			if (registrador[(codigo_op & 0x0F00) >> 8] == (codigo_op & 0x00FF)) {//Pega o n�mero do registrador e valor a ser comparado com o conte�do do registrador
				...
				PC = PC + 2; // Do contr�rio conta apenas a instrucao 0x3XNN
			}
			break;
		...
		case 0x4000:// 0x4XNN: Pula a instrucao se o Registrador [X] for diferente a NN
			if (registrador[(codigo_op & 0x0F00) >> 8] != (codigo_op & 0x00FF)) {//Pega o n�mero do registrador e valor a ser comparado com o conte�do do registrador
				...
				PC = PC + 2; // Do contr�rio conta apenas a instrucao 0x4XNN
			}
		break;
		...
		case 0x5000:// 0x5XY0: Pula a instrucao se o Registrador [X] for diferente do Registrador [Y]
			if (registrador[(codigo_op & 0x0F00) >> 8] == registrador[(codigo_op & 0x00F0) >> 4]) {//Pega os n�mero dos registradores X e Y e compara
				...
				PC = PC + 2; // Do contr�rio conta apenas a instrucao 0x5XY0
			}
			break;
		...
		case 0x8000:
			switch (codigo_op & 0x000F)
			{
				...
				case 0x0004:// 0x8XY4 Soma os registradores [X] e [Y] e guarda em [X]
					...
					if (registrador[(codigo_op & 0x00F0) >> 4] > registrador[(codigo_op & 0x0F00) >> 8]) //Seta o registrador [F] para 1 se tiver carry
						registrador[15] = 1;
				...
				case 0x0005:// 0x8XY5 Subtrai os registradores [Y] de [X] e guarda em [X]. Registrador [15] = 1 se tiver borrow.
					if (registrador[(codigo_op & 0x0F00) >> 4] > registrador[(codigo_op & 0x00F0) >> 8]) //Verifica o conte�do dos registradores [X] e [Y]
				...
				case 0x0007:// 0x8XY7 Subtrai os Registradores [X] de [Y] e guarda em [X]. Registrador [15] = 1 se tiver borrow.
					if (registrador[(codigo_op & 0x0F00) >> 8] > registrador[(codigo_op & 0x00F0) >> 4]) //Verifica o conte�do dos registradores [X] e [Y]
						registrador[15] = 0; //Tem borrow
					else 
						registrador[15] = 1; //Sem borrow
				...
				case 0x000E:// 0x8XYE Seta o Registrador [15] com o ultimo bit do Registrador [X] e joga o Registrador [X]  1 bit � direita
		...
		case 0xF000:
			switch (codigo_op & 0x00FF)
			{
				...
				case 0x0029: // 0xFX29: Pega o valor do timer de delay e joga no Registrador[X]
					//i = i + registrador[(codigo_op & 0x0F00) >> 8];
					i = registrador[(codigo_op & 0x0F00) >> 8];
					PC = PC + 2;
				break;
		...
		{
			if (timer_som == 1)
				al_play_sample(sample, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL); // toca o som caso o timer seja igual � 1
